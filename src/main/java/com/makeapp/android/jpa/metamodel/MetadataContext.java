...
class MetadataContext
{
    ...
    private Map<EntityClass, EntityTypeImpl<?>> entityTypesByEntityClass
            = new HashMap<EntityClass, EntityTypeImpl<?>>();
...
//            = new HashMap<MappedSuperclass, MappedSuperclassTypeImpl<?>>();
    ...
    /**
     * Stack of EntityClass being process. Last in the list is the highest in the stack.
     */
    private List<EntityClass> stackOfEntityClassesBeingProcessed
            = new ArrayList<EntityClass>();
    private Map<MappedSuperclassTypeImpl<?>, EntityClass> mappedSuperClassTypeToEntityClass
            = new HashMap<MappedSuperclassTypeImpl<?>, EntityClass>();
    ...
    /*package*/
    void registerEntityType(EntityClass entityclass, EntityTypeImpl<?> entityType)
    {
        entityTypes.put(entityType.getBindableJavaType(), entityType);
        entityTypesByEntityName.put(entityclass.getEntityName(), entityType);
        entityTypesByEntityClass.put(entityclass, entityType);
        orderedMappings.add(entityclass);
    }
    ...
    /*package*/
//    void registerMappedSuperclassType(MappedSuperclass mappedSuperclass,
...
//        mappedSuperClassTypeToEntityClass.put(mappedSuperclassType, getEntityWorkedOn());
//    }

    /**
     * implementation.  May retur null if the given {@link EntityClass} has not yet been processed.
     *
     * @param EntityClass The Hibernate (config time) metamodel instance representing an entity.
    ...
    {
        return entityTypesByEntityClass.get(EntityClass);
    }
...
////        log.trace("Wrapping up metadata context...");
        ...
        for (Object mapping : orderedMappings) {
            if (EntityClass.class.isAssignableFrom(mapping.getClass())) {
                @SuppressWarnings("unchecked")
                final EntityClass safeMapping = (EntityClass) mapping;
//                log.trace("Starting entity [{}]", safeMapping.getEntityName());
                try {
                    final EntityTypeImpl<?> jpa2Mapping = entityTypesByEntityClass.get(safeMapping);
                    applyIdMetadata(safeMapping, jpa2Mapping);
//                    applyVersionAttribute(safeMapping, jpa2Mapping);
                    List<EntityField> properties =  safeMapping.getFields();
    ...
    private <X> void applyIdMetadata(EntityClass EntityClass, EntityTypeImpl<X> jpaEntityType)
    {
//        if (EntityClass.hasIdentifierProperty()) {
//            final Property declaredIdentifierProperty = EntityClass.getDeclaredIdentifierProperty();
...
//                );
//            }
//        } else if (EntityClass.hasIdentifierMapper()) {
//            jpaEntityType.getBuilder().applyIdClassAttributes(buildIdClassAttributes(jpaEntityType, EntityClass));
//        } else {
//            final KeyValue value = EntityClass.getIdentifier();
...
//        }
    ...
    private <X> void applyVersionAttribute(EntityClass EntityClass, EntityTypeImpl<X> jpaEntityType)
    {
        final Property declaredVersion = EntityClass.getDeclaredVersion();
...
//        @SuppressWarnings("unchecked")
//        Iterator<Property> properties = EntityClass.getIdentifierMapper().getPropertyIterator();
//        while (properties.hasNext()) {
...
//    }
    ...
    {
        stackOfEntityClassesBeingProcessed.add(EntityClass);
    }

    public void popEntityWorkedOn(EntityClass EntityClass)
    {
        final EntityClass stackTop = stackOfEntityClassesBeingProcessed.remove(
                stackOfEntityClassesBeingProcessed.size() - 1
        );
        if (stackTop != EntityClass) {
//            throw new AssertionFailure("Inconsistent popping: "
//                    + EntityClass.getEntityName() + " instead of " + stackTop.getEntityName());
        }
    }

    private EntityClass getEntityWorkedOn()
    {
        return stackOfEntityClassesBeingProcessed.get(
                stackOfEntityClassesBeingProcessed.size() - 1
        );
    }

    public EntityClass getEntityClassHostingProperties(MappedSuperclassTypeImpl<?> mappedSuperclassType)
    {
        final EntityClass EntityClass = mappedSuperClassTypeToEntityClass.get(mappedSuperclassType);
        if (EntityClass == null) {
//            throw new AssertionFailure("Could not find EntityClass for MappedSuperclassType: "
