...
public class AttributeFactory
{
    ...
    public <X, Y> AttributeImplementor<X, Y> buildAttribute(AbstractManagedType<X> ownerType, EntityField property)
    {
...
//        log.trace("Building attribute [{}.{}]", ownerType.getJavaType().getName(), property.getName());
        ...
        if (attributeMetadata.isPlural()) {
            ...
            SingularAttributeImpl attrs = new SingularAttributeImpl<X, Y>(
            ...
            attrs.setProperty(property);
            return attrs;
        }
    }

    private <X> AttributeContext<X> wrap(final AbstractManagedType<X> ownerType, final EntityField property)
    {
     ...
     */
    @SuppressWarnings({"unchecked"})
    public <X, Y> SingularAttributeImpl<X, Y> buildIdAttribute(AbstractIdentifiableType<X> ownerType, EntityField property)
    {
//        log.trace("Building identifier attribute [{}.{}]", ownerType.getJavaType().getName(), property.getName());
     ...
     */
    @SuppressWarnings({"unchecked"})
    public <X, Y> SingularAttributeImpl<X, Y> buildVersionAttribute(AbstractIdentifiableType<X> ownerType, EntityField property)
    {
//        log.trace("Building version attribute [{}.{}]", ownerType.getJavaType().getName(), property.getName());
    ...
    private <Y> Type<Y> getMetaModelType(ValueContext typeContext)
    {
        switch (typeContext.getValueClassification()) {
            case BASIC: {
                return new BasicTypeImpl<Y>(typeContext.getBindableType(), javax.persistence.metamodel.Type.PersistenceType.BASIC);
            }
//            case ENTITY: {
...
//        log.trace("Starting attribute metadata determination [{}]", attributeContext.getPropertyMapping().getName());
        final Member member = attributeContext.getPropertyMapping().getField();
...
//            final EntityField property = attributeContext.getPropertyMapping();
//            final Type.PersistenceType persistenceType = ownerType.getPersistenceType();
