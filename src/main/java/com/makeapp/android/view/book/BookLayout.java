...
public class BookLayout extends FrameLayout
{
    ...
    OnTouchListener touchListener = new OnTouchListener()
    {
        public boolean onTouch(View v, MotionEvent event)
        {
            ...
            if (action == MotionEvent.ACTION_UP && mSelectCorner != BookCorner.None && mState == BookState.TRACKING) {
                if (mState == BookState.ANIMATING)
                    return false;
                if (mSelectCorner == BookCorner.LeftTop) {
                    if (scrollX < contentWidth / 2) {
                        aniStopPos = new Point(0, 0);
                    } else {
                        aniStopPos = new Point(2 * contentWidth, 0);
                    }
                } else if (mSelectCorner == BookCorner.RightTop) {
                    if (scrollX < contentWidth / 2) {
                        aniStopPos = new Point(-contentWidth, 0);
                    } else {
                        aniStopPos = new Point(contentWidth, 0);
                    }
                } else if (mSelectCorner == BookCorner.LeftBottom) {
                    if (scrollX < contentWidth / 2) {
                        aniStopPos = new Point(0, contentHeight);
                    } else {
                        aniStopPos = new Point(2 * contentWidth, contentHeight);
                    }
                } else if (mSelectCorner == BookCorner.RightBottom) {
                    if (scrollX < contentWidth / 2) {
    ...
    public void handleAniEnd(Canvas canvas)
    {
        ...
        if (closeBook) {
            closeBook = false;
            if (mSelectCorner == BookCorner.LeftTop || mSelectCorner == BookCorner.LeftBottom) {
                if (scrollX > contentWidth / 2) {
                    indexPage -= 1;
            ...
            } else if (mSelectCorner == BookCorner.RightTop || mSelectCorner == BookCorner.RightBottom) {
                if (scrollX < contentWidth / 2) {
                    indexPage += 1;
